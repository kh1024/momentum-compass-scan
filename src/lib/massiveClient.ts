  ...
  try {
    const result = await massiveLimiter.run(() => retryWithBackoff<Response>(async () => {
      ...
      if (response.status === 429) {
        ...
        logApiHealth({ endpoint: path, url, method, ticker: options.ticker ?? null, statusCode: 429, statusText: response.statusText || "Too Many Requests", retryAfterMs, responseTimeMs: Date.now() - start, cached: false, retryCount, rateLimited: true, errorMessage: `429 Too Many Requests on ${method} ${path}${retryAfterMs ? ` (retry-after ${retryAfterMs}ms)` : ""}` });
    ...
    logApiHealth({ endpoint: path, url, method, ticker: options.ticker ?? null, statusCode: response.status, statusText: response.statusText, responseTimeMs: Date.now() - start, cached: false, retryCount, rateLimited: false, errorMessage: null });
    ...
    logApiHealth({ endpoint: path, url, method, ticker: options.ticker ?? null, statusCode, statusText, retryAfterMs, responseTimeMs: Date.now() - start, cached: false, retryCount, rateLimited, errorMessage: message });
